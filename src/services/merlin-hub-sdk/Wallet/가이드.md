...
2. **버퍼링 누적 정산형 세션 (예: 금고지기)**
   ...
   * **과금 타이밍**: 브라우저 스토리지(SessionStorage 등)는 유저의 창 닫기, 페이지 새로고침 등으로 유실될 우려가 크며 클라이언트 조작에 취약합니다. 따라서 소모된 실제 토큰량은 **반드시 개별 앱의 신뢰할 수 있는 서버 사이드 임시 저장소(로컬 DB, 인메모리 캐시, 또는 서버리스 데이터 스토어 중 설계 회의를 통해 채택된 스토리지)에 실시간으로 보존(영속화)**해야 합니다. 사용자가 AI 작업을 수행할 때마다 해당 서버 사이드 임시 스토리지에 토큰 사용량을 누적 기록해 두고, **[사용자 로그아웃]** 또는 **[30분간의 무활동으로 인한 세션 만료]** 시점에 백엔드에서 누적된 최종 합산 실소모 토큰 메트릭을 읽어 `chargeDynamic`을 **단 1회** 통합 호출하여 차감한 후 스토리지를 비웁니다.
