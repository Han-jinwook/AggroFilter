...
2. **버퍼링 누적 정산형 세션 (예: 금고지기)**
   ...
   * **과금 타이밍**: 사용자가 AI 작업을 수행할 때마다 소모된 실제 토큰량을 **개별 앱 프론트엔드/백엔드 내부의 임시 스토리지(SessionStorage/로컬메모리 등)에 안전하게 누적(Aggregation)**합니다. 이후 **[사용자 로그아웃]** 또는 **[30분간의 무활동으로 인한 세션 만료]** 시점에 누적된 최종 실소모 토큰 합산량을 가지고 `chargeDynamic`을 **단 1회** 통합 호출하여 차감합니다.
