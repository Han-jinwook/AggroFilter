...
[주제어 헌법 (Topic Constitution) - v3.2]:
모든 주제어는 예외 없이 **"한글 2단어(띄어쓰기 1회 포함)"** 형태로만 존재해야 한다.
- **Why**: 벡터 검색 정밀도 향상 및 UI 일관성(배지 디자인) 확보.
- **Rule**: 3단어 이상(예: "AI 교육 자료")은 입력/생성 즉시 AI가 핵심 2단어("AI 교육")로 압축하여 저장.
...
[어그로성 평가 정책 v2 - 2025.12.27 업데이트]
- **핵심 철학**: '흥미 유발(Hook)'과 '기만(Deception)'의 분리.
- **착한 어그로(Good Hook)**: 제목이 다소 자극적이더라도 본문이 이를 충실히 해소하면 '마케팅 요소'로 인정 (점수 대폭 감경).
- **나쁜 어그로(Bad Clickbait)**: 약속 불이행, 결론 지연, 내용 불일치 시 높은 점수 부여.
- **점수 보정**: 정확성이 80점 이상일 경우, 어그로성 점수를 50% 이상 감경하도록 AI에 지시.
...
### 8.6. 주요 기능 구현 완료 (Major Feature Implementation) - [v3.2 추가, 2025-12-27]

이번 세션(2025-12-27)에서 완료된 핵심 기능 구현 사항입니다.

#### A. 채널 랭킹 페이지 (/p-ranking) 실데이터 연동
- **API 개발**: `app/api/ranking/route.ts` 구현 완료.
  - 검색어(topic) 기반 필터링 지원.
  - 신뢰도 점수(`f_avg_reliability`) 기준 내림차순 정렬.
  - 상위 50개 채널 데이터 반환.
- **UI 연동**:
  - `p-ranking/page.tsx`에서 실제 API 호출 로직 추가.
  - 검색어 입력 시 Debounce(0.5초) 적용으로 API 부하 최소화.
  - 로딩 상태(Loading State) 처리 추가.
  - 신뢰도 점수에 따른 동적 색상(신호등) 로직 적용.

#### B. 청소년 서비스 (Youth Service) 완성
- **프롬프트 고도화**: 나이(6~19세) 입력 시 해당 연령 눈높이에 맞는 설명과 OX 퀴즈를 생성하도록 프롬프트 최적화.
- **UX 개선**: '점 3개' 메뉴를 직관적인 '물음표(?)' 툴팁으로 변경하여 접근성 향상.
- **외부 연동**: 생성된 프롬프트를 클립보드에 복사하고 ChatGPT로 즉시 연결하는 흐름 완성.

#### C. 댓글 시스템 (Comments) 구축
- **DB 연동**: `t_comments` 테이블과 `app/api/comments` API 연동 완료.
- **기능 구현**:
  - 댓글 읽기/쓰기 및 대댓글(답글) 기능 완벽 구현.
  - 작성 즉시 화면에 반영되는 Optimistic UI 적용.
  - 로그인 상태 체크 및 비로그인 시 로그인 모달 트리거.

#### D. 상호작용 (Interactions: 좋아요/싫어요) 구현
- **DB 스키마 대응**: `t_interactions` 테이블 연동 (외래키 `f_analysis_id` 사용).
- **API 개발**: `app/api/interaction/route.ts` 구현.
  - 토글(Toggle) 로직: 이미 누른 상태에서 다시 누르면 취소, 다른 버튼 누르면 변경.
  - 사용자별 상태(userInteraction) 조회 기능 추가.
- **UI 연동**:
  - 로그인한 사용자의 기존 좋아요/싫어요 상태 자동 표시.
  - 클릭 시 즉각적인 숫자 반영 및 색상 변경.

#### E. 보안 강화 (Security)
- **RLS 설정**: Supabase Security Advisor 경고 해결.
  - `t_channel_stats`: 누구나 조회 가능(SELECT true) 정책 적용.
  - `t_interactions`: 외부 API 직접 접근 차단, 서버 사이드 접근만 허용.

#### F. 메인 페이지 (Main Page) 연동
- **플로우 개선**: URL 입력 -> 분석 요청 -> (완료 시) -> **결과 페이지로 즉시 자동 이동** 구현.
- **UX 최적화**: 불필요한 대기 시간(3초 타이머) 제거로 사용자 경험 속도 향상.

#### G. 주제어 시스템 (Topic System) 완비
- **마스터 주제어 구축**: 사용자 제공 660개 표준 주제어 정제 및 DB 적재 완료 (`t_topics_master`).
- **벡터 검색 엔진**: Gemini `text-embedding-004` 모델 기반 768차원 벡터 시스템 구축.
- **하이브리드 주제 선정 로직**:
  1. AI가 1차 주제 제안.
  2. DB에서 벡터 유사도 검색.
  3. **유사도 65% 이상**: 기존 마스터 주제로 자동 표준화 (랭킹 통합).
  4. **유사도 65% 미만**: **신규 마스터 주제로 자동 등극** (DB에 즉시 INSERT하여 661번째 주제로 등록).

#### H. 주제어 데이터 무결성 강화 (Data Integrity)
- **2단어 규칙 강제 적용**: 초기 데이터 중 3단어 이상으로 구성된 36개 주제어 식별 및 AI 기반 압축/병합 처리 완료. (예: "AI 교육 자료" -> "AI 교육")
- **최적화**: 중복 제거 및 병합을 통해 최종 650개 마스터 주제어로 정제 완료.

#### I. 임베딩 충돌 해결 전략 (English Translation Strategy) - [v3.3 Critical Fix]
- **문제**: Gemini `text-embedding-004` 모델이 짧은 한글 단어(예: "AI 농업" vs "AI 전망")를 구별하지 못하고 동일한 벡터를 반환하는 치명적 버그 발견. (Romanization도 의미론적 구분 실패)
- **해결책**: **"영문 번역(English Translation)"** 전략 도입.
- **로직**:
  1. 임베딩 생성 요청 시, 한글 주제어를 영문으로 번역 (예: "자영업 창업" -> "Self-employment Startup").
  2. 번역된 영문 문자열을 API의 `title` 파라미터로 전달.
  3. 영어의 의미적 차이를 이용하여 **고유하고 의미론적으로 정확한 벡터** 생성.
  4. 이를 통해 "베네수엘라"와 "블록체인" 같은 엉뚱한 매칭 문제 해결.

#### J. 주제 선정 로직 고도화 (Macro Category Selection) - [v3.3 Update]
- **문제**: AI가 "스터디카페", "아이폰" 등 지엽적인 소재를 주제로 선정하여 파편화 발생.
- **해결책**:
  1. 프롬프트에 **"구체적 소재 금지, 거시적 분야(Macro Category) 선택"** 지침 추가.
  2. 표준 주제어 목록(Standard Topic List) 제공하여 AI가 이를 참고하도록 유도.
  3. 결과: "스터디카페" -> **"자영업 창업"**, "베네수엘라 사태" -> **"국제 정세"**로 표준화.

---

**[v3.3 업데이트 완료일: 2025-01-03]**
