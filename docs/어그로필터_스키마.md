...
> [!WARNING]
> **알림 시스템 통합에 따른 테이블 사용 제한 (Deprecated)**
> - `t_notifications` 및 `t_channel_subscriptions` 일부 컬럼(`f_notification_enabled` 등)은 Merlin Hub SDK의 통합 알림 모듈(`useHubNotifier`) 및 중앙 데이터베이스 `family_notifications`로 이전 통합되었습니다.
> - 따라서 기존 어그로필터 로컬 DB의 `t_notifications` 테이블은 신규 알림 발송 시 사용하지 않으며, 보존 및 호환성을 위해서만 유지(Deprecated)됩니다.
