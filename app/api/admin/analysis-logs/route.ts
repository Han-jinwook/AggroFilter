...
// REFACTORED_BY_MERLIN_HUB: t_users JOIN(분석 로그) ??app_aggro_profiles ?��? ?�정
...
export async function PATCH(request: Request) {
  try {
    ...
    try {
      ...
      if (res.rowCount === 0) {
        return NextResponse.json({ error: '?�당 분석 기록??찾을 ???�습?�다' }, { status: 404 });
      }
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!Array.isArray(ids) || ids.length === 0) {
      return NextResponse.json({ error: 'ids 배열???�요?�니?? }, { status: 400 });
    }
