...
export async function POST(request: Request) {
  try {
    const { userId, channelName, channelId, channelThumbnail, isEntered, categoryName } = await request.json();

    if (!userId || !channelName || isEntered === undefined) {
      return NextResponse.json({ error: 'Missing required fields' }, { status: 400 });
    }

    const statusIcon = isEntered ? '?éâ' : '?ìâ';
    const statusText = isEntered ? 'ÏßÑÏûÖ' : '?àÎùΩ';
    ...
    try {
      ...
      await client.query(
        ...
        [
          userId,
          'top10_change',
          `${channelName} Ï±ÑÎÑê???ÅÏúÑ 10%??${statusText}?àÏäµ?àÎã§`,
