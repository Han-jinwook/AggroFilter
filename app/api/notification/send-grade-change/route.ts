...
export async function POST(request: Request) {
  try {
    ...
    const gradeInfo: Record<string, { color: string; label: string; icon: string }> = {
      'Green': { color: '#10b981', label: '?†Î¢∞ (Green Zone)', icon: 'Ôø? },
      'Yellow': { color: '#f59e0b', label: 'Ï£ºÏùò (Yellow Zone)', icon: '?ü°' },
      'Red': { color: '#ef4444', label: 'Í≤ΩÍ≥† (Red Zone)', icon: '?î¥' }
    };
    ...
    try {
      ...
      await client.query(
        ...
        [
          userId,
          'grade_change',
          `${channelName} Ï±ÑÎÑê???†Î¢∞???±Í∏â??Î≥ÄÍ≤ΩÎêò?àÏäµ?àÎã§ (${oldGrade} ??${newGrade})`,
