...
// REFACTORED_BY_MERLIN_HUB: t_users ?¨Î†à??Ï∂©Ï†Ñ ??Hub wallet ?¥Í? ?àÏ†ï
...
export async function POST(request: Request) {
  try {
    ...
    // Supabase ?úÎ≤Ñ ?∏Ï¶ù ???§Ìå® ??body.userId fallback
    ...
    if (!Number.isFinite(credits) || credits <= 0 || credits > 10000) {
      return NextResponse.json({ error: '?†Ìö®?òÏ? ?äÏ? ?¨Î†à???òÎüâ?ÖÎãà??' }, { status: 400 });
    }
    ...
    try {
      ...
      await client.query(
        ...
        [userId, credits, newBalance, `Mock Ï∂©Ï†Ñ +${credits}C`]
      );
