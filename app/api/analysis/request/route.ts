  ...
  try {
      ...
      try {
        ...
        if (existingAnalysis.rows.length > 0) {
          ...
          if (!forceRecheck && (row.f_processing_stage === 'speed_ready' || row.f_processing_stage === 'pending')) {
            ...
            if (isFreshInFlight) {
              console.log('?´ë? ì§„í–‰ ì¤‘ì¸ ë¶„ì„?…ë‹ˆ?? ê¸°ì¡´ ë¶„ì„ ID ?¬ì‚¬??', row.f_id, row.f_processing_stage);
              ...
              return NextResponse.json({
                message: '?´ë? ë¶„ì„??ì§„í–‰ ì¤‘ì…?ˆë‹¤. ê¸°ì¡´ ê²°ê³¼ë¥?ë¶ˆëŸ¬?µë‹ˆ??',
                analysisId: row.f_id,
                cached: true,
            ...
            console.warn('stale pending/speed_ready ë¶„ì„ ê°ì?: ??ë¶„ì„?¼ë¡œ ?¬ì‹œ??, {
              analysisId: row.f_id,
    ...
    try {
      ...
      const runFullTrack = async () => {
        try {
          ...
          console.warn('[Full Track] 1ì°??¤íŒ¨, ?ë™ ?¬ì‹œ??ì§„í–‰:', {
            status: rawStatus,
            message: rawMessage,
          });
      ...
      if (speedOutcome.ok) {
        ...
        console.warn('[Speed Track] ?¤íŒ¨ - Full Trackë¡?ê³„ì† ì§„í–‰', {
          name: speedErr?.name,
      ...
      try {
        ...
        console.error('AI ?¤íŒ¨ stage ?€???¤íŒ¨:', failedUpdateError);
      }
      ...
      try {
        ...
        try {
          ...
          if (fallbackRes.rows.length > 0) {
            ...
            console.warn('[AI Fallback] ìµœì‹  ?„ë£Œ ë¶„ì„ ?¬ì‚¬??', fallbackAnalysisId);
            ...
            return NextResponse.json({
              message: 'AI ë¶€?˜ë¡œ ê¸°ì¡´ ê²€ì¦?ê²°ê³¼ë¥??°ì„  ?œê³µ?©ë‹ˆ??',
        ...
        console.error('Fallback ë¶„ì„ ì¡°íšŒ ?¤íŒ¨:', fallbackError);
      }
      ...
      const wrapped: any = new Error(
        isQuotaError
          ? 'AI ?”ì²­???¼ì‹œ?ìœ¼ë¡?ëª°ë ¤ ë¶„ì„??ì§€?°ë˜ê³??ˆìŠµ?ˆë‹¤. ? ì‹œ ???¤ì‹œ ?œë„?´ì£¼?¸ìš”.'
          : isTimeoutError
            ? 'AI ?‘ë‹µ ?œê°„??ì´ˆê³¼?˜ì—ˆ?µë‹ˆ?? ? ì‹œ ???¤ì‹œ ?œë„?´ì£¼?¸ìš”.'
            : 'AI ë¶„ì„ ì¤??¤ë¥˜ê°€ ë°œìƒ?ˆìŠµ?ˆë‹¤. ? ì‹œ ???¤ì‹œ ?œë„?´ì£¼?¸ìš”.'
      );
    ...
    if (!isValidTarget) {
      ...
      try {
        ...
        console.error('[AI Reject] DB ?€???¤íŒ¨:', invalidErr);
      } finally {
    ...
    try {
      ...
      if (!shouldKeepParentOnDecrease) {
        ...
        // 5-2. ë¶„ì„ ê²°ê³¼ 2ì°??…ë°?´íŠ¸ (v2.0 ?„ë“œ ë°˜ì˜) - f_topic ?œê±°
        await client.query(`
          UPDATE t_analyses
          SET f_video_url = $2,
              f_video_id = $3,
