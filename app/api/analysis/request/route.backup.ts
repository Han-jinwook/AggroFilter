...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isRecheck) {
      ...
      // REFACTORED_BY_MERLIN_HUB: t_users 코인 → Hub wallet 이관 예정
      ...
      try {
        ...
        if (!Number.isFinite(credits) || credits <= 0) {
          return NextResponse.json({ error: '코인이 부족합니다.' }, { status: 402, headers: corsHeaders });
        }
      } finally {
    ...
    try {
      ...
      // REFACTORED_BY_MERLIN_HUB: t_users ALTER TABLE 제거 — 코인은 Hub wallet로 이관 예정
