...
export async function GET(request: Request) {
  ...
  // Ranking Key ?ùÏÑ±: lang_categoryId ?êÎäî ?∏Ïñ¥ ?ÑÏ≤¥
  ...
  try {
    ...
    try {
      // t_rankings_cache?êÏÑú Ï°∞Ìöå
      let query: string;
      let queryParams: any[];
      
      if (rankingKey) {
        // ?πÏ†ï Ïπ¥ÌÖåÍ≥†Î¶¨Îß?Ï°∞Ìöå
        query = `
          SELECT 
        ...
        // ?∏Ïñ¥Î≥??ÑÏ≤¥ Ï°∞Ìöå (Ï±ÑÎÑêÎ≥?ÏµúÍ≥† ?êÏàò Í∏∞Ï?)
        query = `
      ...
      const rankedChannels = res.rows.map((row) => ({
        id: row.id,
        rank: row.rank, // Ï∫êÏãú?êÏÑú Í∞Ä?∏Ïò® rank ?¨Ïö©
        name: row.name,
        avatar: row.avatar,
