...
export default function PlazaPage() {
  ...
  const allAnalyzedVideos: VideoData[] = useMemo(
    () => [
      {
        date: "25.02.07",
...
function HotIssueCard({
  ...
  const getScoreColor = () => {
    if (type === "aggro") {
      // For Aggro: High score is Bad (Red), Low score is Good (Green)
      if (item.score >= 80) return "text-red-500"
      if (item.score <= 40) return "text-green-500"
      return "text-amber-500"
    }
    // For Trust (default): High score is Good (Green), Low score is Bad (Red)
  ...
  const handleShare = (e: React.MouseEvent) => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        url: `${window.location.origin}/result?id=${item.id || 1}`,
      })
    } else {
      navigator.clipboard.writeText(`${window.location.origin}/result?id=${item.id || 1}`)
      alert("링크가 복사되었습니다!")
    }
  }
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <div className="text-right">
          ...
          {type === "trust" && (
            <div className="flex flex-col items-end">
              <span className="text-xs font-bold text-slate-400">신뢰도</span>
              <span className={`text-base font-black ${getScoreColor()}`}>{item.score}</span>
            </div>
          )}
          {type === "aggro" && (
            <div className="flex flex-col items-end">
              <span className="text-xs font-bold text-slate-400">어그로</span>
              <span className={`text-base font-black ${getScoreColor()}`}>{item.score}</span>
            </div>
          )}
        </div>
...
function HotChannelCard({
  item,
  type,
  label = "신뢰도",
}: { item: any; type: "views" | "trust" | "controversy"; label?: string }) {
  const handleShare = (e: React.MouseEvent) => {
    e.preventDefault()
    e.stopPropagation()
    if (navigator.share) {
      navigator.share({
        title: `${item.name} 채널`,
        text: `${item.name} 채널 - 신뢰도 ${item.score}점`,
        url: `${window.location.origin}/channel/${item.id || 1}`,
      })
    } else {
      navigator.clipboard.writeText(`${window.location.origin}/channel/${item.id || 1}`)
      alert("링크가 복사되었습니다!")
    }
  }
  ...
  const getScoreColor = () => {
    if (type === "controversy") {
      // For Controversy: High score is Bad (Red), Low score is Good (Green)
      if (item.score >= 80) return "text-red-500"
      if (item.score <= 40) return "text-green-500"
      return "text-amber-500"
    }
