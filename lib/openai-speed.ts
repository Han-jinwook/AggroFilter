...
export async function analyzeContentSpeed(
  ...
  const prompt = `
    ...
    3. **타임스탬프 요약 가이드 (절대 규칙)**:
        ...
        - **요약 밀도 가이드**: 영상 **10분당 약 2~3개**의 챕터 분할을 권장하며, 전체 맥락이 촘촘히 연결되게 하라. (처음과 끝만 요약하는 것은 지양)
        - **종료 시점 일치**: 요약의 마지막 타임스탬프는 반드시 제공된 영상의 전체 길이(duration) 또는 자막의 마지막 시점과 일치해야 한다.
        - **형식**: '0:00 - 소주제: 요약내용' (특수문자/마크다운 금지).
        - **가변 분할**: 영상 길이에 따라 요약 개수를 유연하게 조절하라.

    ## ⚠️ 썸네일 스포일러 (Thumbnail Spoiler) — 황금비율 규칙 (V2)
    너의 임무는 영상 전체 요약이 아니다. 
    오직 **제목과 썸네일 이미지가 시청자에게 던진 '궁금증(떡밥)'**을 1~3개 추출하고, 해당 떡밥에 대해 영상에서 언급된 모든 내용을 종합하여 결론을 공개하라.

    ### 목적
    시청자가 클릭 전 궁금해하는 "제목/썸네일의 그 내용이 진짜인가? 결론이 무엇인가?"에 대해 영상의 해당 구간 내용을 종합 요약하여 핀셋 응답한다.
