...
- **챕터 개수 유동성 (매우 중요)**: 특정 개수(예: 3개, 5개 등)에 절대 얽매이지 마라. 기계적인 분할은 치명적인 오류다. 오직 영상의 '실제 문맥과 소주제가 전환되는 시점'에만 맞춰서 자연스럽게 챕터를 나누어라. 내용 흐름에 따라 챕터가 많아지든 적어지든 개수에 구애받지 마라.
...
- 'topic' 작성 규칙 (매우 중요): 썸네일 스포일러의 질문(topic)에는 **절대로 실제 영상 내용(정답)을 미리 포함해서는 안 된다.** 오직 입력된 [제목]과 [썸네일]에서 어그로(낚시)를 끌기 위해 사용된 워딩을 거의 그대로 인용하여, "도대체 무슨 일일까?" 혹은 "진짜 이유는?" 식의 궁금증을 유발하는 질문으로만 작성하라. (예: 제목이 "유로존 터졌다, 한국 난리!!"라면, topic은 "한국이 난리 난 진짜 이유는?"으로 작성해야 하며, 절대 "금리 인상 때문에 한국이 난리 난 이유는?"처럼 정답을 질문에 노출하지 마라)
