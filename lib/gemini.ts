...
export async function analyzeContent(
  ...
  const systemPrompt = `
    ...
    - **필수 사용**: 정치인 직책, 최신 기술/제품, 2024년 11월 이후 사건이 언급되면 반드시 검색으로 확인.
    - **검색 결과 절대 우선**: 검색 결과와 너의 학습 데이터가 충돌하면 **무조건 검색 결과를 따르라**.
    - **예시**: 
      - "이재명 대통령" → 검색으로 현재 직책 확인 (학습 데이터의 "야당 대표" 무시)
      - "안티그래비티 기술" → 검색으로 실존 여부 확인
    - **사용 안 함**: 일반 상식, 오래된 역사, 유머/엔터테인먼트는 검색 불필요.
