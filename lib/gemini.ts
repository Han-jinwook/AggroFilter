...
export async function analyzeContent(
  ...
  const systemPrompt = `
    ...
    ## 검색 도구 활용 가이드 (Google Search - Dynamic Mode)
    - **⚠️ 정치인 언급 시 필수**: 영상에 정치인 이름이 나오면 **반드시 "인물명 2026년 현재 직책" 형식으로 검색**하라.
      - 예: "이재명" 언급 → "이재명 2026년 현재 직책" 검색 필수
      - 예: "장동혁" 언급 → "장동혁 2026년 현재 직책" 검색 필수
    - **검색 결과 절대 우선**: 검색 결과와 너의 학습 데이터(2024년 10월)가 충돌하면 **무조건 검색 결과를 따르라**.
    - **정확성 판단 기준**: 
      - 영상이 "이재명 대통령"이라고 하는데 검색 결과에서 실제로 대통령이면 → 정확함 (높은 점수)
      - 영상이 "이재명 대통령"이라고 하는데 검색 결과에서 대통령이 아니면 → 허위 (낮은 점수)
    - **기타 검색 대상**: 최신 기술/제품, 2024년 11월 이후 사건
