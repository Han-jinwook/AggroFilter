...
export async function analyzeContent(
  ...
  const systemPrompt = `
    ...
    **[평가 이유(evaluationReason) 작성 강제 규칙 - 매우 중요]**
    ...
    ### **짧은 영상(Shorts) 및 브랜드/제품 키워드 보정 규칙**
    1. **짧은 영상(60초 이하)**: 시간 제약상 제목의 모든 키워드를 본문에서 직접 언급하기 어려울 수 있다. 핵심 주제가 영상 내용과 부합하면 키워드 미언급만으로 정확성을 크게 감점하지 마라.
    2. **브랜드/제품명 판단 절대 규칙 (매우 중요)**:
        - 너의 학습 데이터에 특정 제품/서비스명이 없더라도, **"존재하지 않는 제품"이나 "허위 키워드"로 단정하지 마라.** 기술 업계는 빠르게 변하며, 너의 학습 컷오프 이후에 출시된 제품일 수 있다.
        - 제목의 키워드가 영상에서 다루는 기술·서비스의 **동일 기업/생태계**에 속하면(예: 구글 제품명 + 구글 AI 기술 영상), 해당 키워드는 맥락적으로 관련 있는 것으로 간주하라.
        - "내가 모르는 제품이다 → 허위다"라는 논리는 **절대 금지**한다. 대신 "확인 불가"로 처리하고, 정확성 점수에 소폭 불확실성만 반영하라.
    3. **자막 부재·극소량 영상**: 자막이 없거나 극히 짧은 경우, 썸네일과 제목만으로 판단해야 하므로 정확성 점수에 불확실성을 반영하되, 어그로 점수는 확인 가능한 근거에만 기반하라.
