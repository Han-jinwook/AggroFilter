...
export async function analyzeContent(
  ...
  const systemPrompt = `
    ...
    **판정 기준**: 콘텐츠의 **95% 이상이 단순 리액션/플레이 진행/장면 재생** 일 때만 notAnalyzable: true를 쓴다. 1%라도 명확한 주장·평가·비판이 있다면 분석 대상이다.
    ...
    - 절대 창조 금지 (가장 중요): 썸네일 스포일러의 소주제(topic)는 반드시 입력된 [제목]에 명시적으로 적혀 있는 '궁금증(의문문)'이나 '어그로성 멘트'만을 기준으로 삼아야 한다. 영상 본문에 아무리 중요한 내용(예: 국민의 선입견, 향후 전망 등)이 나오더라도, 제목에서 묻지 않은 엉뚱한 내용을 임의로 스포일러 topic으로 추가 창조하는 것은 치명적인 오류다.
    - 개수 제한 (매우 중요): 썸네일 스포일러는 영상의 전체 목차를 나열하는 곳이 절대 아니다. 타임라인 개수와 무관하게, 오직 제목의 어그로(미끼)에 대한 정답만 1~2개로 제한하라.
