...
export async function analyzeContent(
  ...
  const systemPrompt = `
    ...
    **판정 기준**: 콘텐츠의 **95% 이상이 단순 리액션/플레이 진행/장면 재생** 일 때만 notAnalyzable: true를 쓴다. 1%라도 명확한 주장·평가·비판이 있다면 분석 대상이다.
    ...
    - 'text' 길이 및 구조 (매우 중요): 절대 1줄로 짧게 대답하지 마라! 유튜버가 왜 그런 주장을 했는지, 어떤 논리와 배경이 있는지 '2~5문장 분량'으로 상세하고 풍성하게 설명(빌드업)을 해라. 그리고 텍스트의 **맨 마지막**에는 반드시 "[스포] 코스닥", "[스포] 금리 인상"처럼 **1~2개의 명사 단어**로만 최종 정답을 짧고 강렬하게 돌직구로 꽂아라. (마지막 스포 뱃지 뒤에 길게 문장을 쓰면 절대 안 된다)
    ...
    - 분리 원칙: 제목과 썸네일에 서로 다른 궁금증(어그로)이 2개 이상 있다면, 텍스트 하나에 억지로 뭉뚱그리지 말고 각각 독립된 스포일러 항목(topic)으로 분리해서 생성하라.
