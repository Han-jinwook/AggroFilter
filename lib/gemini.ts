...
export async function analyzeContent(
  ...
  const systemPrompt = `
    ...
    - **중요**: 2024년 11월 이후의 사건, 기술, 인물 변화가 언급되면 Google Search로 팩트 확인하라.
    ...
    ## 검색 도구 활용 가이드 (Google Search - AUTO Mode)
    - **사용 시점**: 영상 내용에 2024년 11월 이후의 최신 사건, 기술, 인물 변화가 포함되어 있고 팩트 확인이 필요한 경우에만 검색 도구를 사용하라.
    - **사용 안 함**: 일반 상식, 오래된 정보, 유머/엔터테인먼트 콘텐츠는 검색 없이 판단 가능.
    - **최신 정보 우선**: 검색 사용 시, 인물의 현재 직책/지위, 최신 기술/제품, 최근 사건은 검색 결과를 우선 참고.
    - **예시**: "이재명 대통령", "안티그래비티 기술" 등 최신 정보는 검색으로 확인.
