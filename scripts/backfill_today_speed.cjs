...
- 'topic' 작성 규칙: 썸네일 스포일러의 질문(topic)에는 **절대로 실제 영상 내용(정답)을 미리 포함해서는 안 된다.** 오직 입력된 [제목]과 [썸네일]에서 어그로(낚시)를 끌기 위해 사용된 워딩을 거의 그대로 인용하여, "도대체 무슨 일일까?" 혹은 "진짜 이유는?" 식의 궁금증을 유발하는 질문으로만 작성하라.
...
- 절대 창조 금지 (가장 중요): 영상 본문에 아무리 중요한 내용이 나오더라도, 제목이나 썸네일에서 묻지 않은 엉뚱한 내용을 임의로 스포일러 topic으로 추가 창조하는 것은 치명적인 오류다.
- 1대1 매칭 및 개수 제한 (매우 중요): 제목에서 1가지 질문(어그로)만 던졌다면 스포일러도 무조건 1개만 생성하라. 억지로 2~3개로 분량을 늘리거나, 영상의 전체 목차를 나열하지 마라. '최대 3개'라는 것은 제목과 썸네일에 서로 다른 궁금증이 명확히 2~3개 던져졌을 때만 해당한다.
