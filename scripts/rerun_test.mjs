...
async function analyze(channelName, title, transcript, userLanguage, thumbnailUrl) {
  const prompt = `
    ...
    ### 작성 규칙 (엄격)
    - **출처 표기 필수**: 반드시 구체적인 출처 유형으로 문장을 시작하라.
...
async function main() {
  const ids = ['9396c092-dd3e-4c8e-8f09-c5a99c2fdf26', 'e883a7e1-5943-4f24-a1dd-368408cc2e6e'];
  ...
  console.log(`Processing ${res.rows.length} videos...`);

  for (const row of res.rows) {
    try {
      console.log(`Analyzing: ${row.f_title} (${row.f_id})`);
      ...
      if (result.subtitleSummary && result.thumbnail_spoiler) {
        ...
        console.log(`  Summary: ${result.subtitleSummary}`);
        console.log(`  Spoiler: ${JSON.stringify(result.thumbnail_spoiler, null, 2)}`);
      }
    } catch (err) {
      console.error(`  Error analyzing ${row.f_id}:`, err);
    }
  }
